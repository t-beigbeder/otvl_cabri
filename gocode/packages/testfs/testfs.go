...
type plugin struct {
	...
	osFileWriteString func(fp *os.File, content string) (n int, err error)
	ioutilReadFile    func(path string) (content []byte, err error)
}
...
func (tfs *Fs) RandTextFile(path string, mxlines int) error {
	...
	defer rtf.Close()
	for i, max := 0, tfs.rand.Intn(mxlines)+1; i < max; i++ {
		if _, err = tfs.osFileWriteString(rtf, tfs.RandomWord()+"\n"); err != nil {
			return err
		}
	}
	return nil
...
func (f *Fs) SetOsCreate(osCreate func(path string) (*os.File, error)) {
	if f.writable {
		f.plugin.osCreate = osCreate
	} else {
		panic("SetOsCreate is forbidden")
	}
}

func (f *Fs) SetOsFileWriteString(osFileWriteString func(*os.File, string) (n int, err error)) {
	if f.writable {
		f.plugin.osFileWriteString = osFileWriteString
	} else {
		panic("SetOsFileWriteString is forbidden")
	}
}

func (f *Fs) SetIoutilReadFile(ioutilReadFile func(path string) ([]byte, error)) {
	if f.writable {
		f.plugin.ioutilReadFile = ioutilReadFile
	} else {
		panic("SetIoutilReadFile is forbidden")
	}
}
